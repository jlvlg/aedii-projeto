...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc;) {
        switch (atoi(argv[i])) {
            ...
            case 2:
                if (!strcmp(argv[i+1], "int")) {
                    root = avl.remove(root, types.Int(atoi(argv[i+2])), &changes, types.copy);
                } else if (!strcmp(argv[i+1], "str")) {
                    root = avl.remove(root, types.String(argv[i+2]), &changes, types.copy);
                } else if (!strcmp(argv[i+1], "char")) {
                    root = avl.remove(root, types.Char(argv[i+2][0]), &changes, types.copy);
                }
                i += 3;
                break;
            case 3:
